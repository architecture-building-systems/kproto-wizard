...
def show_switch_kprototyper_session_ui():
    ...
    selected_key = st.selectbox("Select session", list(sessions.keys()), format_func=lambda k: labels[k], key="kp_switch_select")
...
def show_manage_kprototyper_session_ui():
    ...
    selected = st.selectbox("Manage session", list(sessions.keys()), key="kp_manage_select")
